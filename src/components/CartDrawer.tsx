      ...
      {isOpen && (
        <>
          ...
          >
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center border border-outline-variant/30">
                            <button 
                              ...
                              className="p-1 md:p-1.5 hover:bg-surface transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-tertiary"
                            >
                              <Minus className="w-2.5 h-2.5 md:w-3 md:h-3 text-primary" aria-hidden="true" />
                            </button>
                            <span className="w-6 md:w-8 text-center text-[10px] md:text-xs font-bold text-primary" aria-label={`Quantity: ${item.quantity}`}>
                              {item.quantity}
                            </span>
                            <button 
            ...
            {cart.length > 0 && (
              <div className="p-6 md:p-8 bg-surface-container-low border-t border-outline-variant/30 space-y-4 md:space-y-6">
                <div className="space-y-1.5 md:space-y-2" role="region" aria-label="Order summary">
                  <div className="flex justify-between text-on-surface-variant text-xs md:text-sm">
                    <span>Subtotal</span>
                  ...
                  <div className="flex justify-between text-primary font-headline text-xl md:text-2xl pt-3 md:pt-4 border-t border-outline-variant/20">
                    <span>Total</span>
                    <span>₹{cartTotal.toLocaleString()}</span>
                  </div>
                </div>
                <button className="w-full bg-primary text-white py-4 md:py-5 text-[9px] md:text-[10px] uppercase tracking-[0.3em] font-bold hover:bg-tertiary transition-all flex items-center justify-center gap-2 md:gap-3 shadow-xl focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-tertiary">
                  Proceed to Checkout <ArrowRight className="w-3.5 h-3.5 md:w-4 md:h-4" aria-hidden="true" />
                </button>
                <p className="text-[8px] md:text-[9px] text-center text-outline uppercase tracking-widest">
